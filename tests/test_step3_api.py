...
class _FakeGeminiAdapter:
    def generate(
        ...
        *,
        ...
        if "Use horizontal writing mode for all translated replacement text" not in prompt:
            raise AssertionError("Step 3 prompt lost the horizontal writing constraint")
        if "Never use vertical text or vertical text layout under any circumstances" not in prompt:
            raise AssertionError("Step 3 prompt lost the vertical-ban constraint")
