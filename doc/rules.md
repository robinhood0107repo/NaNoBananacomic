...
- `main`과 `dev`를 같은 안정 커밋으로 맞춘 뒤에도, 후속 개선은 다시 `codex/...` 브랜치에서 시작한다.

현재 운영 메모:

- `main`과 `dev`는 최근 안정 커밋 기준으로 동기화된 상태를 유지한다.
- detector 기반 말풍선 정합 같은 실험/개선 작업은 `codex/...` feature branch에서만 진행한다.
