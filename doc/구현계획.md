...
- OCR, 번역, 식질, 말풍선 내부 인페이팅은 Nano Banana가 담당한다.
- 최종적으로 원본 페이지와 Nano Banana 결과를 안전하게 합성해 완성 페이지를 만든다.
...
flowchart LR
  ...
  B --> C1["BW manga detector"]
  B --> C2["color/3D detector"]
  C1 --> D["mask refine"]
  C2 --> D["mask refine"]
...
- Nano Banana 결과 해상도가 원본과 다르면 즉시 실패
...
- color/3D route 구현
- `ogkalu` 계열 detector + SAM 2 refine
- profile별 benchmark 도입
- 페이지별 profile override UI 연결
...
- 흑백 일본 만화:
  - `huyvux3005/manga109-segmentation-bubble` 주력
...
- 컬러/3D comic 쪽은 명확한 특화 segmentation 모델을 찾지 못했다.
- 따라서 컬러/3D는 처음부터 별도 fine-tuning 가능성을 열어두고 설계해야 한다.
