...
- API 응답 또는 수동 웹 결과 해상도가 원본과 다르면 즉시 실패
...
- `2026-04-03` 기준 alpha restore backend/CLI 1차 구현 완료
...
- 현재 1차 구현의 align은 global resize 수준이며, 말풍선 위치 정밀 정합 관점에서는 임시 대응이다
- 다음 강화 방향은 `manual import registration` 단계 추가다
  - imported balloon support 추정
  - reference support는 `balloons_only_rgba.png` 또는 `balloon_union_mask.png`
  - scale/translation registration 우선
  - 필요 시 affine/ECC fallback 검토
- final composite 구현은 아직 미착수
- outside-mask diff validator 구현은 아직 미착수
- GUI 단계 버튼과 결과 가져오기 패널 연결은 아직 미착수

## Phase 5

- Grounded SAM 2 fallback 조사 및 연결 여부 재판정
- CVAT/FiftyOne 기반 hard case 수집
- 필요 시 fine-tuning harness 추가
- DVC experiment 루프 연결
